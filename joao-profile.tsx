...
function JoaoWeekPlan(props: CommonProps) {
  ...
  return <div className="joao-stack">
    ...
    {isPreStart ? <section className="joao-prestart compact"><CalendarClock size={28} /><div><span>SEMANA LIVRE</span><h2>Nenhuma atividade programada</h2><p>O perfil João começa em 24/08/2026. Esta semana não recebe tarefas, não gera atraso e não consome saldo.</p></div></section> : <>
      ...
      {week.tasks.length > 0 && <section className="joao-panel joao-daily-planner"><div className="joao-panel-title"><div><span>ORGANIZAÇÃO ARRASTÁVEL</span><h2>Distribua os estudos pelos dias da semana</h2><p>Arraste qualquer assunto para o dia desejado. A escolha é sua e não transforma a agenda em obrigação fixa.</p></div></div><div className="joao-unscheduled-drop" onDragOver={event => event.preventDefault()} onDrop={() => scheduleTask()}><strong>Sem dia definido</strong><span>{week.tasks.filter(task => !task.scheduledDate).length} atividades livres</span>{week.tasks.filter(task => !task.scheduledDate).map(task => <article key={task.id} draggable onDragStart={() => setDragged(task.id)}><GripVertical size={13} /><b className={task.priority.toLowerCase()}>{task.priority}</b><span>{task.title}</span></article>)}</div><div className="joao-day-grid">{weekDays.map(date => { const dayTasks = week.tasks.filter(task => task.scheduledDate === date); const dayDate = new Date(`${date}T12:00:00`); return <section className={`joao-day-column ${date === today ? "today" : ""}`} key={date} onDragOver={event => event.preventDefault()} onDrop={() => scheduleTask(date)}><header><strong>{dayDate.toLocaleDateString("pt-BR", { weekday: "short" }).replace(".", "")}</strong><span>{dayDate.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })}</span><small>{dayTasks.reduce((sum, task) => sum + task.estimatedHours, 0).toFixed(1)}h</small></header>{dayTasks.length ? dayTasks.map(task => <article key={task.id} draggable onDragStart={() => setDragged(task.id)}><div><GripVertical size={12} /><b className={task.priority.toLowerCase()}>{task.priority}</b></div><strong>{task.title}</strong><small>{task.estimatedHours}h · {task.source}</small><button onClick={() => updateTask(task.id, item => ({ ...item, scheduledDate: undefined }))}><X size={11} /> Remover do dia</button></article>) : <p>Arraste um assunto para cá</p>}</section>; })}</div></section>}
    </>}
