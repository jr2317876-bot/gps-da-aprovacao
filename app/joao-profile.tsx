...
export function JoaoWeeklyWorkspace({ view, profileId, setToast, onSaveStatus }: { view: string; profileId: string; setToast: (message: string) => void; onSaveStatus: (status: "loading" | "saving" | "saved" | "error") => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const load = async () => {
        ...
        if (cancelled) return;
        const parsed = (result?.data?.data as JoaoState | undefined) ?? localData ?? emptyState();
  ...
  useEffect(() => {
    ...
    saveQueue.current = saveQueue.current.then(async () => {
      const { error } = await client.from("profile_states").upsert({ profile_id: profileId, scope: "joao_weekly_v1", data: state, updated_at: new Date().toISOString() });
      onSaveStatus(error ? "error" : "saved");
      if (error) setToast(`Falha ao salvar o plano semanal: ${error.message}`);
    });
  }, [state, hydrated, profileId, onSaveStatus, setToast]);

  const updateState = useCallback((recipe: (previous: JoaoState) => JoaoState) => setState(previous => recipe(previous)), []);
  const coverage = theoreticalCoverage(state);
  ...
  if (view === "Hoje") return <JoaoHome {...common} currentWeek={currentWeek} coverage={coverage} status={status} />;
  if (view === "Meu plano") return <JoaoWeekPlan {...common} />;
  if (view === "Plano Mestre") return <JoaoMasterPlan state={state} coverage={coverage} status={status} today={today} />;
  if (view === "Mentoria") return <JoaoMentorship {...common} coverage={coverage} status={status} />;
  if (view === "Questões") return <JoaoQuestions {...common} />;
  if (view === "Flashcards") return <JoaoFlashcardRecommendations state={state} />;
  if (view === "Assuntos") return <JoaoTopics state={state} updateState={updateState} setToast={setToast} />;
  if (view === "Prioridades") return <JoaoPriorities />;
  if (view === "Desempenho") return <JoaoPerformance state={state} coverage={coverage} status={status} />;
  if (view === "Simulados") return <JoaoSimulations state={state} updateState={updateState} setToast={setToast} today={today} />;
  if (view === "Revisões") return <JoaoReviews state={state} />;
  if (view === "Bancas e metas") return <JoaoGoals coverage={coverage} status={status} state={state} />;
  return <JoaoHome {...common} currentWeek={currentWeek} coverage={coverage} status={status} />;
}
...
function JoaoWeekPlan(props: CommonProps) {
  ...
  function removeFacultyTopic(value: string) {
    updateState(previous => { const base = previous.weeks[selectedWeek] ?? emptyWeek(selectedWeek); return { ...previous, weeks: { ...previous.weeks, [selectedWeek]: { ...base, facultyTopics: base.facultyTopics.filter(item => item !== value) } } }; });
  }
  ...
  function removeTask(task: WeekTask) {
    ...
    updateState(previous => {
      ...
      return { ...previous, weeks: { ...previous.weeks, [selectedWeek]: { ...base, tasks: tasks.map((item, order) => ({ ...item, order })), completion: weekCompletion(tasks), status: "Em andamento", closed: false } } };
    });
...
function JoaoQuestions({ state, updateState, setToast, today }: CommonProps) {
  const [form, setForm] = useState({ topicId: "", questions: 20, accuracy: 60, security: 5, questionNeed: 5, flashcardNeed: 5, errorCause: "Não sabia o conteúdo" as ErrorCause });
  ...
  function save() {
    ...
    const log: DetailedQuestionLog = { id: editingId ?? `q-${today}-${sequence}`, date: today, topic: topic.title, area: topic.area, ...form, topicId: topic.id, reviewMode: derived.mode };
    updateState(previous => {
      ...
      const progress = { ...previous.progress, [topic.id]: { ...current, questions: true, accuracy: form.accuracy, security: form.security, questionNeed: form.questionNeed, flashcardNeed: form.flashcardNeed, errorCause: form.errorCause, reviewMode: derived.mode, reviewStatus: derived.status, flashcardsRequired: derived.flashcardsRequired, lastContact: today } };
      const weeks = Object.fromEntries(Object.entries(previous.weeks).map(([key, week]) => [key, { ...week, tasks: week.tasks.map(task => task.topicId === topic.id ? { ...task, questions: true, flashcardsRequired: derived.flashcardsRequired, reviewMode: derived.mode } : task) }]));
      return { ...previous, questionLogs, progress, weeks };
    });
    setForm({ topicId: "", questions: 20, accuracy: 60, security: 5, questionNeed: 5, flashcardNeed: 5, errorCause: "Não sabia o conteúdo" }); setEditingId(null);
    setToast(`Resultado salvo. Próxima recomendação: ${derived.mode}.`);
  }
  function edit(log: DetailedQuestionLog) { setForm({ topicId: log.topicId, questions: log.questions, accuracy: log.accuracy, security: log.security, questionNeed: log.questionNeed, flashcardNeed: log.flashcardNeed, errorCause: log.errorCause }); setEditingId(log.id); }
  function remove(id: string) { updateState(previous => ({ ...previous, questionLogs: previous.questionLogs.filter(log => log.id !== id) })); if (editingId === id) setEditingId(null); setToast("Registro excluído. O histórico restante foi preservado."); }
  return <div className="joao-stack"><JoaoHeader eyebrow="QUESTÕES" title="Registro que ensina o GPS" text="Faça o bloco sem consulta, com material fechado, e corrija apenas depois de finalizar. As metas são sempre editáveis." /><div className="joao-metrics"><JoaoMetric label="Questões registradas" value={String(total)} note="tema → subárea → grande área" /><JoaoMetric label="Acerto ponderado" value={total ? `${weighted}%` : "—"} note="considera o volume realizado" /><JoaoMetric label="Recomendação atual" value={derived.mode} note={derived.status === "Urgente" ? "revisão urgente" : derived.status === "Em breve" ? "revisar em breve" : "consolidado"} /></div><div className="joao-two-columns questions"><section className="joao-panel joao-question-form"><div className="joao-panel-title"><div><span>{editingId ? "EDITAR REGISTRO" : "NOVO BLOCO"}</span><h2>Como foi seu desempenho?</h2></div></div><label>Assunto<select value={form.topicId} onChange={event => setForm({ ...form, topicId: event.target.value })}><option value="">Selecione</option>{matrixTopics.map(item => <option value={item.id} key={item.id}>{item.area} · {item.title}</option>)}</select></label><div className="joao-form-grid"><label>Questões<input type="number" min="1" value={form.questions} onChange={event => setForm({ ...form, questions: Number(event.target.value) })} /></label><label>Acerto (%)<input type="number" min="0" max="100" value={form.accuracy} onChange={event => setForm({ ...form, accuracy: Number(event.target.value) })} /></label><label>Segurança (0–10)<input type="number" min="0" max="10" value={form.security} onChange={event => setForm({ ...form, security: Number(event.target.value) })} /></label><label>Revisar por questões (0–10)<input type="number" min="0" max="10" value={form.questionNeed} onChange={event => setForm({ ...form, questionNeed: Number(event.target.value) })} /></label><label>Revisar por flashcards (0–10)<input type="number" min="0" max="10" value={form.flashcardNeed} onChange={event => setForm({ ...form, flashcardNeed: Number(event.target.value) })} /></label><label>Principal causa<select value={form.errorCause} onChange={event => setForm({ ...form, errorCause: event.target.value as ErrorCause })}>{["Não sabia o conteúdo", "Esqueci informação", "Confundi conceitos", "Erro de raciocínio/conduta", "Desatenção"].map(item => <option key={item}>{item}</option>)}</select></label></div><div className={`joao-recommendation ${derived.status.toLocaleLowerCase("pt-BR").replace(" ", "-")}`}><BrainCircuit size={20} /><div><strong>{derived.mode}</strong><p>{derived.flashcardsRequired ? "Flashcards indicados somente para os erros de memória." : form.errorCause === "Desatenção" ? "Não será criada revisão ampla nem flashcard desnecessário." : "O próximo bloco será ajustado pelo tipo de dificuldade."}</p></div></div><button className="joao-primary full" onClick={save}><Check size={16} /> {editingId ? "Atualizar registro" : "Salvar e recalcular"}</button></section><section className="joao-panel"><div className="joao-panel-title"><div><span>HISTÓRICO</span><h2>Blocos realizados</h2><p>Editar ou excluir recalcula somente o perfil João.</p></div></div>{state.questionLogs.length ? <div className="joao-question-history">{state.questionLogs.map(log => <article key={log.id}><div className={log.accuracy < 60 ? "low" : log.accuracy < 75 ? "mid" : "high"}><strong>{log.accuracy}%</strong><small>{log.questions} questões</small></div><div><strong>{log.topic}</strong><span>{log.area} · segurança {log.security}/10</span><small>{log.errorCause} · {log.reviewMode}</small></div><div><button onClick={() => edit(log)}><Pencil size={13} /></button><button onClick={() => remove(log.id)}><Trash2 size={13} /></button></div></article>)}</div> : <div className="joao-empty"><ListChecks size={22} /><strong>Nenhum bloco registrado.</strong></div>}</section></div></div>;
}

function JoaoFlashcardRecommendations({ state }: { state: JoaoState }) {
  const indicated = matrixTopics.filter(topic => state.progress[topic.id]?.flashcardsRequired && !state.progress[topic.id]?.flashcards);
  return <div className="joao-stack"><JoaoHeader eyebrow="FLASHCARDS" title="Indicações a partir dos seus erros" text="O GPS não fornece cartões. Você cria ou revisa na outra plataforma somente quando as questões revelam falha de memória." /><section className="joao-flash-rule"><Layers3 size={28} /><div><h2>Flashcards não são obrigação depois da teoria</h2><p>Esquecimento de critérios, doses, classificações, valores e confusão conceitual podem gerar indicação. Erro de raciocínio prioriza questões; desatenção não cria revisão desnecessária.</p></div></section><section className="joao-panel"><div className="joao-panel-title"><div><span>FILA INDICADA</span><h2>{indicated.length} assuntos precisam de flashcards dos erros</h2></div></div>{indicated.length ? <div className="joao-revision-cards">{indicated.map(topic => <article key={topic.id}><b>{topic.priority}</b><div><span>{topic.area} · {topic.subarea}</span><strong>{topic.title}</strong><p>Criar/revisar flashcards apenas dos erros identificados nas questões.</p></div><em>{state.progress[topic.id]?.reviewStatus}</em></article>)}</div> : <div className="joao-empty"><Check size={24} /><strong>Nenhuma indicação de flashcards no momento.</strong><p>Isso é normal antes dos primeiros blocos de questões.</p></div>}</section></div>;
}
...
function JoaoSimulations({ state, updateState, setToast, today }: { state: JoaoState; updateState: (recipe: (previous: JoaoState) => JoaoState) => void; setToast: (message: string) => void; today: string }) {
  const [form, setForm] = useState({ bank: "IAMSPE" as "IAMSPE" | "SES-PE", questions: 100, accuracy: 0 });
  function save() { if (form.questions <= 0 || form.accuracy < 0 || form.accuracy > 100) return setToast("Confira os dados do simulado."); updateState(previous => ({ ...previous, simulations: [{ id: `sim-${Date.now()}`, date: today, ...form }, ...previous.simulations] })); setToast("Simulado registrado e incorporado ao acompanhamento."); }
  return <div className="joao-stack"><JoaoHeader eyebrow="SIMULADOS" title="Provas completas no momento certo" text="Até o fim de 2027, a prioridade é construir a base. Em 2028, provas antigas e simulados IAMSPE/SES-PE tornam-se predominantes." /><section className="joao-roadmap"><article><span>2026–2027</span><strong>Base + questões por tema</strong><p>Progressão para subárea e grande área conforme consolidação.</p></article><ChevronRight /><article><span>Dez/2027</span><strong>Buffer e consolidação</strong><p>Resolver pendências sem criar uma nova corrida de conteúdo.</p></article><ChevronRight /><article><span>2028</span><strong>Simulados completos</strong><p>IAMSPE e SES-PE, provas antigas, diretrizes e correção de fraquezas.</p></article></section><div className="joao-two-columns"><section className="joao-panel joao-question-form"><div className="joao-panel-title"><div><span>REGISTRAR</span><h2>Resultado de prova completa</h2></div></div><label>Banca<select value={form.bank} onChange={event => setForm({ ...form, bank: event.target.value as "IAMSPE" | "SES-PE" })}><option>IAMSPE</option><option>SES-PE</option></select></label><div className="joao-form-grid"><label>Questões<input type="number" min="1" value={form.questions} onChange={event => setForm({ ...form, questions: Number(event.target.value) })} /></label><label>Acerto (%)<input type="number" min="0" max="100" value={form.accuracy} onChange={event => setForm({ ...form, accuracy: Number(event.target.value) })} /></label></div><button className="joao-primary full" onClick={save}><Check size={16} /> Salvar resultado</button></section><section className="joao-panel"><div className="joao-panel-title"><div><span>HISTÓRICO</span><h2>{state.simulations.length} simulados</h2></div></div>{state.simulations.length ? <div className="joao-sim-list">{state.simulations.map(item => <article key={item.id}><b>{item.accuracy}%</b><div><strong>{item.bank}</strong><span>{item.questions} questões · {item.date.split("-").reverse().join("/")}</span></div></article>)}</div> : <div className="joao-empty"><ClipboardCheck size={22} /><strong>Nenhum simulado registrado.</strong></div>}</section></div></div>;
}

function JoaoReviews({ state }: { state: JoaoState }) {
